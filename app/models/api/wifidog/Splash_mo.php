...
class Splash_mo extends CI_Model {
	...
	public function __construct(){
		...
		$this->data = array('state'=>'success', 'reson'=>'', 'tmp_page'=>'');
	}

	public function index (){

		$gw_id 			= $this->input->get('gw_id');
		$gw_address		= $this->input->get('gw_address');
		$gw_port		= $this->input->get('gw_port');
		$url 			= urlencode($_GET['url']);
		$clientip    	= $this->input->get('client_ip');
		$clientmac 		= $this->input->get('client_mac');
		$wanip	  		= $this->input->get('wanip');
		$rnd 			= $this->input->get('rnd');
		$key 			= $this->input->get('key');
		$msg 			= $this->input->get('msg');

		if($gw_id && $gw_address && $gw_port){
			try {
				$query = "select a.*, ac.* from ap a, apconfig ac where a.apid=ac.apid and a.apname = ?";
				...
				if($ap['bindmac'] == '1'){
					...
					if ($res->num_rows() > 0) {
						...
						if($result['validatetime'] > time()){
							$this->db->query ("update user set state = '1' and hearttime=? where uid = ?", array(time(),$result['uid']));
							...
							$query = "update bindmac set valid ='0' where apid = ? and mac = ?";
							$this->db->query ($query, array($ap['apid'], $clientmac));
						}
					}
				}
				...
				if ($acctype){ 
					...
					if($acctype == 'mobileauthcode'){
						try{
							...
							$res = $this->db->query($query, array($ap['smstid'], $ap['cid']));
							if(!$res || $res->num_rows () == 0){
